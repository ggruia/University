# Presupunem ca avem costul de mutare al unui bloc egal cu indicele in alfabet, cu indicii incepand de la 1
# (care se calculează prin 1+ diferenta dintre valoarea codului ascii al literei blocului de mutat si codul ascii al literei "a" ).
# Astfel A* ar trebui sa prefere drumurile in care se muta intai blocurile cu informatie mai mica lexicografic pentru a ajunge la una dintre starile scop
...
class Graph: #graful problemei
    ...
    def calculeaza_h(self, infoNod, tip_euristica="euristica banala"):
        if tip_euristica=="euristica banala":
                        ...
                        for iElem, elem in enumerate(stiva):
                            try:
                                ...
                                if elem!=scop[iStiva][iElem]:
                                    h += ord(elem) - 96 #adun cu costul minim pe o mutare (adica costul lui a)
                            except IndexError:
            ...
            #calculez cate blocuri nu sunt la locul lor fata de fiecare dintre starile scop, si apoi iau minimul dintre aceste valori
            euristici=[]
            for scop in self.scopuri:
                h=0
                for iStiva, stiva in enumerate(infoNod):
...
def a_star(gr, nrSolutiiCautate, euristica):
    k = nrSolutiiCautate
    ...
    while len(c)>0:
        ...
        if gr.testeaza_scop(nodCurent):
            print("Solutia {}: ".format(k - nrSolutiiCautate + 1))
            nodCurent.afisDrum(afisCost=True, afisLung=True)
            print("\n----------------\n")
            nrSolutiiCautate-=1
            if nrSolutiiCautate==0:
                return
        lSuccesori=gr.genereazaSuccesori(nodCurent,tip_euristica=euristica)	
        for s in lSuccesori:
            i=0
            gasit_loc=False
...
nrSolutiiCautate=3
a_star(gr, nrSolutiiCautate=3,euristica="euristica admisibila 1")